...
function handleRelay(peer_id) {
  ...
  let incomingLAN = idToInfo[peer_id][1].split(".");
  incomingLAN = incomingLAN[0] + "." + incomingLAN[1] + "." + incomingLAN[2];

  if (incomingLAN === myLAN) {
    console.log(`relaying ${streamsToRelay.length} streams to ${peer_id}`);
    for (let i = 0; i < streamsToRelay.length; i++) {
      streamsToRelay[i].getTracks().forEach((track) => {
        peers[peer_id].addTrack(track, streamsToRelay[i]);
      });
    }
  } else {
    console.log(`relaying stream of ${peer_id} to ${lanIDs.length} peers`);
    for (let i = 0; i < lanIDs.length; i++) {
      if (lanIDs[i] !== myID && lanIDs[i] !== "") {
        streams[peer_id].getTracks().forEach((track) => {
          peers[lanIDs[i]].addTrack(track, streams[peer_id]);
        });
      }
    }
  }
