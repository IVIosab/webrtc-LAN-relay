...
def read_csv(file_path):
    ...
    with open(file_path, "r") as csvfile:
        ...
        for row in csvreader:
            ...
            data_dict[peer]["bytes_sent"] = eval(row["BytesSent"])
            data_dict[peer]["bytes_sent_in_bits/s"] = eval(row["BytesSent_in_bits/s"])
            data_dict[peer]["bytes_received"] = eval(row["BytesReceived"])
            data_dict[peer]["bytes_received_in_bits/s"] = eval(
                row["BytesReceived_in_bits/s"]
            )
    return data_dict
...
def get_time(data_dict, length):
    diff = length - len(data_dict["bytes_sent_in_bits/s"])
