...
io.sockets.on('connection', (socket) => {
    ...
    socket.on('relayICECandidate', (config) => handleIceCandidate(socket, config));
    socket.on('relaySessionDescription', (config) => handleSessionDescription(socket, config));
});
...
function createPeerConnection(socket, peerId, channel, shouldCreateOffer) {
    ...
    socket.emit('addPeer', { 'peer_id': peerId, 'should_create_offer': shouldCreateOffer });
    channels[channel][peerId].emit('addPeer', { 'peer_id': socket.id, 'should_create_offer': !shouldCreateOffer });
