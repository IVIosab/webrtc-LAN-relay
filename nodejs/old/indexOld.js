...
function init() {
    ...
    signaling_socket.on('addPeer', config => {
        ...
        peer_connection.ontrack = event => {
            //check if event kind is video, if not, return
            ...
            peer_media_streams[peer_id] = event.streams[0];
            console.log("Got Track for " + peer_id);
            for (var peer in peers) {
                for (var streamPeer in peer_media_streams) {
                    if (peer !== streamPeer) {
                        console.log("Adding " + streamPeer + "'s stream to " + peer);
                        for (track in peer_media_streams[streamPeer].getTracks()) {
                            peers[peer].addTrack(peer_media_streams[streamPeer].getTracks()[track], peer_media_streams[streamPeer]);
                        }
                    }
                }
            }
        ...
        for (track in local_media_stream.getTracks()) {
            peer_connection.addTrack(local_media_stream.getTracks()[track], local_media_stream);
        }
